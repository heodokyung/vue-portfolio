  ...
  <AppSection>
    <AppContainer class="detail-layout">
      <aside class="detail-layout__aside">
        ...
        />
        <div v-if="project.links?.length" class="project-links" aria-label="프로젝트 관련 링크">
          <a
      ...
      <article class="detail-layout__content flow">
        ...
        <section v-if="project.solution.length">
          <h2>해결 방법</h2>
          <ul class="check-list">
            <li v-for="item in project.solution" :key="item">{{ item }}</li>
          </ul>
        </section>
        <section v-if="project.result.length">
          <h2>결과</h2>
          <ul class="check-list">
            <li v-for="item in project.result" :key="item">{{ item }}</li>
          </ul>
        </section>
        <section v-if="project.media.length">
          <h2>이미지</h2>
